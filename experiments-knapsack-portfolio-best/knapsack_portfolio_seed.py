...
def select_projects(
    ...
    Uses brute force enumeration of all subsets to find optimal solution.
    """
    n = len(projects)
    # Precompute costs and element sets
    costs = [int(p["cost"]) for p in projects]
    elem_sets = [frozenset(int(e) for e in p["elements"]) for p in projects]
        ...
        for i in range(n):
            if mask & (1 << i):
                ...
                if total_cost > budget:
                    break
                covered.update(elem_sets[i])
        
        if total_cost <= budget:
            score = sum(element_values[e] for e in covered)
            if score > best_score:
                best_score = score
                best_subset = [i for i in range(n) if mask & (1 << i)]
